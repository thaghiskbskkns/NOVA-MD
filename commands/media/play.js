/**
 * Play Command - Download audio from YouTube as document (MP3 file)
 */
...
module.exports = {
  name: 'play',
  aliases: ['mp3', 'docaudio'],
  category: 'media',
  description: 'Download audio from YouTube as document (MP3 file)',
  ...
  async execute(sock, msg, args, extra) {
    try {
      ...
      if (!text) {
        return await extra.reply(`📌 Usage: ${config.prefix}play <song name or YouTube link>\n\nExample: ${config.prefix}play Believer Imagine Dragons`);
      }
      ...
      const fileName = `${(video.title || 'audio').replace(/[^\w\s-]/g, '')}.${finalExtension}`;
